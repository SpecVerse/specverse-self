...
/**
 * ComponentRelationshipService class
 */
export class ComponentRelationshipService {
  ...
  public async handleChildAdded(params: any = {}): Promise<any> {
    ...
    console.log('[ComponentRelationshipService] Processing ChildAdded', params);
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(`[ComponentRelationshipService] handleChildAdded failed:`, error);
      throw error;
    }
  }
  ...
  public async handleChildRemoved(params: any = {}): Promise<any> {
    ...
    console.log('[ComponentRelationshipService] Processing ChildRemoved', params);
    // Delegate to model-specific logic
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(`[ComponentRelationshipService] handleChildRemoved failed:`, error);
      throw error;
    }
  }
  ...
  public async validateRelationshipIntegrity(params: any = {}): Promise<any> {
    try {
          return await prisma.$transaction(async (tx) => {
    // === PRECONDITIONS ===
    // Guard: Component exists
    const component = await prisma.component.findUnique({ where: { id: params.id } });
    if (!component) {
      throw new Error('Precondition failed: Component exists');
    }
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(`[ComponentRelationshipService] validateRelationshipIntegrity failed:`, error);
      throw error;
    }
  }
  ...
  public async repairRelationshipIntegrity(params: any = {}): Promise<any> {
    ...
    const issues = await prisma.component.findMany({
      where: { id: params.id }
    });
    ...
    if (process.env.NODE_ENV === 'development') {
      ...
      console.error(`[ComponentRelationshipService] repairRelationshipIntegrity failed:`, error);
      throw error;
    }
  }
...
export const componentRelationshipService = new ComponentRelationshipService();
export default componentRelationshipService;
