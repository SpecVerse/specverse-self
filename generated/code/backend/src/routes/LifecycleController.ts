import { FastifyInstance, FastifyRequest, FastifyReply } from 'fastify';

/**
 * LifecycleController Routes
 * Generated from SpecVerse specification
 *
 * Model: Lifecycle
 * Operations: create, retrieve, list, update, delete, validate, , , 
 */
export default async function LifecycleRoutes(
  fastify: FastifyInstance,
  options: any
) {
  const handler = options.controllers.LifecycleController;

  // create Lifecycle
  fastify.post('/', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const lifecycle = await handler.create(request.body as any);
        return reply.status(201).send(lifecycle);
      } catch (error) {
        return reply.status(400).send({
          error: 'Failed to create lifecycle',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // retrieve Lifecycle
  fastify.get('/:id', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { id } = request.params as { id: string };
        const lifecycle = await handler.retrieve(id);
      
        if (!lifecycle) {
          return reply.status(404).send({ error: 'Lifecycle not found' });
        }
      
        return reply.send(lifecycle);
      } catch (error) {
        return reply.status(500).send({
          error: 'Failed to retrieve lifecycle',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // list Lifecycle
  fastify.get('/', {
    handler: async (_request: FastifyRequest, reply: FastifyReply) => {
      try {
        const lifecycles = await handler.retrieveAll();
        return reply.send(lifecycles);
      } catch (error) {
        return reply.status(500).send({
          error: 'Failed to list lifecycles',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // update Lifecycle
  fastify.put('/:id', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { id } = request.params as { id: string };
        const lifecycle = await handler.update(id, request.body as any);
        return reply.send(lifecycle);
      } catch (error) {
        return reply.status(400).send({
          error: 'Failed to update lifecycle',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // delete Lifecycle
  fastify.delete('/:id', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { id } = request.params as { id: string };
        await handler.delete(id);
        return reply.status(204).send();
      } catch (error) {
        return reply.status(500).send({
          error: 'Failed to delete lifecycle',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // validate Lifecycle
  fastify.post('/validate', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const { data, operation: op } = request.body as { data: any; operation: string };
        const result = handler.validate(data, { operation: op });
        return reply.send(result);
      } catch (error) {
        return reply.status(400).send({
          error: 'Validation failed',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // attachProfile Lifecycle
  fastify.get('/attach-profile', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const result = await handler.attachProfile(request.body as any);
        return reply.send(result);
      } catch (error) {
        return reply.status(400).send({
          error: 'Failed to execute attachProfile',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // detachProfile Lifecycle
  fastify.get('/detach-profile', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const result = await handler.detachProfile(request.body as any);
        return reply.send(result);
      } catch (error) {
        return reply.status(400).send({
          error: 'Failed to execute detachProfile',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
  
  // hasProfile Lifecycle
  fastify.get('/has-profile', {
    handler: async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        const result = await handler.hasProfile(request.body as any);
        return reply.send(result);
      } catch (error) {
        return reply.status(400).send({
          error: 'Failed to execute hasProfile',
          message: error instanceof Error ? error.message : String(error)
        });
      }
    }
  });
}
