...
function ModelFormView() {
  ...
  // Fetch single model if selected
  const { model, create, update, delete: deleteModel, validate, isDeleting, isValidating } = useModel(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all models for the list
  const { models, isLoading: listLoading } = useModel({ list: true });
  ...
  return (
    <div className="view-modelformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Model' : 'Create Model'}
          </h2>
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No models yet</p>
              <p className="text-sm text-gray-500">Create your first model using the form above</p>
            </div>
          ) : (
                  ...
                  {models?.map((model: Model) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === model.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
