...
function EntityRegistryDetailView() {
  ...
  // Fetch all entities
  const { entityregistrys, isLoading: isLoadingEntityRegistry } = useEntityRegistry({ list: true });
  ...
  const modelData = useMemo(() => ({
    EntityRegistry: (entityregistrys || []).map((item: any) => ({ id: item.id, data: item }))
  }), [entityregistrys]);
  
  // Entity selection
  const entities = entityregistrys || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.EntityRegistry?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingEntityRegistry) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'EntityRegistry' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["EntityRegistry"], name: 'EntityRegistryDetailView' },
    modelData,
    modelSchemas,
