  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/componentform" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
          + New Component
        </Link>
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          ...
          <p className="text-sm mt-1">Create your first component to get started</p>
        </div>
      ) : (
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link to={`/componentdetail?id=${item.id}`} className="text-blue-600 hover:text-blue-800 mr-3">View</Link>
                    <Link to={`/componentform?id=${item.id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>
