  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/modelform" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
          + New Model
        </Link>
      </div>
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">extends</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{formatCell(item.extends)}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link to={`/modeldetail?id=${item.id}`} className="text-blue-600 hover:text-blue-800 mr-3">View</Link>
                    <Link to={`/modelform?id=${item.id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>
