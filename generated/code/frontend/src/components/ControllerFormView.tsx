...
function ControllerFormView() {
  ...
  // Fetch single controller if selected
  const { controller, create, update, delete: deleteController, validate, isDeleting, isValidating } = useController(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all controllers for the list
  const { controllers, isLoading: listLoading } = useController({ list: true });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteController(selectedId);
  ...
  const handleSelectController = (id: string) => {
    setSelectedId(id);
  };
  ...
  return (
    <div className="view-controllerformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Controller' : 'Create Controller'}
          </h2>
          ...
          {listLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-400">Loading controllers...</p>
              </div>
            </div>
          ) : controllers?.length === 0 ? (
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No controllers yet</p>
              <p className="text-sm text-gray-500">Create your first controller using the form above</p>
            </div>
          ) : (
                  ...
                  {controllers?.map((controller: Controller) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === controller.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
