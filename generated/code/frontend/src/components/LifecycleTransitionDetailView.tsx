...
function LifecycleTransitionDetailView() {
  ...
  // Entity selection
  const entities = lifecycletransitions || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.LifecycleTransition?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingLifecycleTransition) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'LifecycleTransition' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["LifecycleTransition"], name: 'LifecycleTransitionDetailView' },
    modelData,
    modelSchemas,
