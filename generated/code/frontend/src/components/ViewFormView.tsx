...
function ViewFormView() {
  ...
  // Fetch single view if selected
  const { view, create, update, delete: deleteView, validate, isDeleting, isValidating } = useView(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all views for the list
  const { views, isLoading: listLoading } = useView({ list: true });
  ...
  return (
    <div className="view-viewformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit View' : 'Create View'}
          </h2>
          ...
          {listLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="text-center">
                ...
                <p className="text-gray-400">Loading views...</p>
              </div>
            </div>
          ) : views?.length === 0 ? (
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No views yet</p>
              <p className="text-sm text-gray-500">Create your first view using the form above</p>
            </div>
          ) : (
                  ...
                  {views?.map((view: View) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === view.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
