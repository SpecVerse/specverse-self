...
function BehaviourSpecDetailView() {
  ...
  const { invariants, isLoading: isLoadingInvariant } = useInvariant({ list: true });
  ...
  const modelData = useMemo(() => ({
    ...
    Invariant: (invariants || []).map((item: any) => ({ id: item.id, data: item }))
  }), [behaviourspecs, invariants]);
  
  // Entity selection
  const entities = behaviourspecs || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.BehaviourSpec?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingBehaviourSpec || isLoadingInvariant) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'BehaviourSpec' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["BehaviourSpec","Invariant"], name: 'BehaviourSpecDetailView' },
    modelData,
    modelSchemas,
