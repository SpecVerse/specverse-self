...
function InvariantDetailView() {
  ...
  const selectedEntity = useMemo(
    () => modelData.Invariant?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingInvariant) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Invariant' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Invariant"], name: 'InvariantDetailView' },
    modelData,
    modelSchemas,
