...
function InstanceFactoryDetailView() {
  ...
  }), [instancefactorys, capabilitymappings]);
  
  // Entity selection
  const entities = instancefactorys || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.InstanceFactory?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingInstanceFactory || isLoadingCapabilityMapping) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'InstanceFactory' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["InstanceFactory","CapabilityMapping"], name: 'InstanceFactoryDetailView' },
    modelData,
    modelSchemas,
