...
function ControllerDetailView() {
  ...
  }), [controllers, operations, events]);
  
  // Entity selection
  const entities = controllers || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.Controller?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingController || isLoadingOperation || isLoadingEvent) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Controller' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Controller","Operation","Event"], name: 'ControllerDetailView' },
    modelData,
    modelSchemas,
