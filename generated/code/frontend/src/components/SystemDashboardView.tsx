...
function SystemDashboardView() {
  ...
  // Fetch entities
  const { specifications, isLoading, error } = useSpecification({ list: true });
  ...
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'dashboard', model: 'Specification' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'dashboard', model: 'Specification', name: 'SystemDashboardView' },
    modelData,
    modelSchemas,
