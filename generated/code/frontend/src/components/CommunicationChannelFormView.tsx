...
function CommunicationChannelFormView() {
  ...
  useEffect(() => {
    fetch('/api/deployments').then(r => r.json()).then(d => setDeploymentOptions(d)).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/communicationChannels/${id}` : '/api/communicationChannels';
      ...
      navigate('/communicationChannellist');
    } catch (e: any) { setError(e.message); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!id || !confirm('Delete this communicationChannel?')) return;
    await fetch(`/api/communicationChannels/${id}`, { method: 'DELETE' });
    navigate('/communicationChannellist');
  };
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} CommunicationChannel</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">channelType</label>
          <input
            type="text"
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/communicationChannellist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
