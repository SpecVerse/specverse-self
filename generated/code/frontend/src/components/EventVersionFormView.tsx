...
function EventVersionFormView() {
  ...
  useEffect(() => {
    fetch('/api/events').then(r => r.json()).then(d => setEventOptions(d)).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/eventVersions/${id}` : '/api/eventVersions';
      ...
      navigate('/eventVersionlist');
    } catch (e: any) { setError(e.message); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!id || !confirm('Delete this eventVersion?')) return;
    await fetch(`/api/eventVersions/${id}`, { method: 'DELETE' });
    navigate('/eventVersionlist');
  };
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} EventVersion</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">deprecated</label>
          <input
            type="checkbox"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">deprecationMessage</label>
          <input
            type="text"
            value={form.deprecationMessage ?? ''}
            onChange={e => setForm({...form, deprecationMessage: e.target.value})}
            className="w-full px-3 py-2 border rounded-md focus:ring-blue-500 focus:border-blue-500"
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/eventVersionlist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
