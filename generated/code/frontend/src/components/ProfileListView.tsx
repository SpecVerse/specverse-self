  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/profileform" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
          + New Profile
        </Link>
      </div>
              ...
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link to={`/profiledetail?id=${item.id}`} className="text-blue-600 hover:text-blue-800 mr-3">View</Link>
                    <Link to={`/profileform?id=${item.id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>
