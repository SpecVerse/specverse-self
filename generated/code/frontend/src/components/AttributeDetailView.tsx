...
function AttributeDetailView() {
  ...
  const selectedEntity = useMemo(
    () => modelData.Attribute?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Attribute' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Attribute"], name: 'AttributeDetailView' },
    modelData,
    modelSchemas,
