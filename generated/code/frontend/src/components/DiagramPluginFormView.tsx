...
function DiagramPluginFormView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/diagramPlugins/${id}` : '/api/diagramPlugins';
      ...
      navigate('/diagramPluginlist');
    } catch (e: any) { setError(e.message); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!id || !confirm('Delete this diagramPlugin?')) return;
    await fetch(`/api/diagramPlugins/${id}`, { method: 'DELETE' });
    navigate('/diagramPluginlist');
  };
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} DiagramPlugin</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">diagramType</label>
          <input
            type="text"
            value={form.diagramType ?? ''}
            onChange={e => setForm({...form, diagramType: e.target.value})}
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/diagramPluginlist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
