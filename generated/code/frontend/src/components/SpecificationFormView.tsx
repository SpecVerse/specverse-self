 ...
 * Form view for creating and editing Specification
 *
 * Model: Specification
 * Type: form
 */
function SpecificationFormView() {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  
  // Fetch single specification if selected
  const { specification, create, update, delete: deleteSpecification, validate, isDeleting, isValidating } = useSpecification(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all specifications for the list
  const { specifications, isLoading: listLoading } = useSpecification({ list: true });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteSpecification(selectedId);
  ...
  return (
    <div className="view-specificationformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No specifications yet</p>
              <p className="text-sm text-gray-500">Create your first specification using the form above</p>
            </div>
          ) : (
                  ...
                  {specifications?.map((specification: Specification) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === specification.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
                      {/* Table cells will be dynamically generated based on model attributes */}
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => handleSelectSpecification(specification.id)}
...
export default SpecificationFormView;
