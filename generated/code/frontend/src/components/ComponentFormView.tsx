...
/**
 * ComponentFormView
 * Form view for creating and editing Component
 *
 * Model: Component
 * Type: form
 */
function ComponentFormView() {
  ...
  // Fetch single component if selected
  const { component, create, update, delete: deleteComponent, validate, isDeleting, isValidating } = useComponent(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all components for the list
  const { components, isLoading: listLoading } = useComponent({ list: true });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteComponent(selectedId);
  ...
  return (
    <div className="view-componentformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Component' : 'Create Component'}
          </h2>
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No components yet</p>
              <p className="text-sm text-gray-500">Create your first component using the form above</p>
            </div>
          ) : (
                  ...
                  {components?.map((component: Component) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === component.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
