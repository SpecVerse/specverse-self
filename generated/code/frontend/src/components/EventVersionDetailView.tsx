...
function EventVersionDetailView() {
  ...
  // Fetch all entities
  const { eventversions, isLoading: isLoadingEventVersion } = useEventVersion({ list: true });
  ...
  const modelData = useMemo(() => ({
    EventVersion: (eventversions || []).map((item: any) => ({ id: item.id, data: item }))
  }), [eventversions]);
  
  // Entity selection
  const entities = eventversions || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.EventVersion?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingEventVersion) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'EventVersion' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["EventVersion"], name: 'EventVersionDetailView' },
    modelData,
    modelSchemas,
