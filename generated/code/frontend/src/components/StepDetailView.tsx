...
function StepDetailView() {
  ...
  const selectedEntity = useMemo(
    () => modelData.Step?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingStep || isLoadingOperation) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Step' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Step","Operation"], name: 'StepDetailView' },
    modelData,
    modelSchemas,
    primaryModel: 'Step',
    selectedEntity,
