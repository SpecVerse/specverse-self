...
function AIOrchestratorFormView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/aIOrchestrators/${id}` : '/api/aIOrchestrators';
      ...
      navigate('/aIOrchestratorlist');
    } catch (e: any) { setError(e.message); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!id || !confirm('Delete this aIOrchestrator?')) return;
    await fetch(`/api/aIOrchestrators/${id}`, { method: 'DELETE' });
    navigate('/aIOrchestratorlist');
  };
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} AIOrchestrator</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">providerType</label>
          <input
            type="text"
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/aIOrchestratorlist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
