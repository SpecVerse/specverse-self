...
function ViewDetailView() {
  ...
  const { layouts, isLoading: isLoadingLayout } = useLayout({ list: true });
  ...
  const selectedEntity = useMemo(
    () => modelData.View?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingView || isLoadingLayout || isLoadingUIComponent || isLoadingEvent) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'View' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["View","Layout","UIComponent","Event"], name: 'ViewDetailView' },
    modelData,
    modelSchemas,
    primaryModel: 'View',
    selectedEntity,
