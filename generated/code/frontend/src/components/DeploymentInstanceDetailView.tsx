...
function DeploymentInstanceDetailView() {
  ...
  // Entity selection
  const entities = deploymentinstances || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.DeploymentInstance?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingDeploymentInstance) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'DeploymentInstance' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["DeploymentInstance"], name: 'DeploymentInstanceDetailView' },
    modelData,
    modelSchemas,
