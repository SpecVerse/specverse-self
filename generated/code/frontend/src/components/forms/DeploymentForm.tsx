/**
 * DeploymentForm
 * Form component for creating/editing Deployment
 */
...
interface DeploymentFormProps {
  ...
  onValidate?: (data: DeploymentFormData) => void;
  onDelete?: () => void;
  onCancel?: () => void;
  isDeleting?: boolean;
 ...
 */
export function DeploymentForm({ deployment, onSubmit, onValidate, onDelete, onCancel, isDeleting, isValidating }: DeploymentFormProps) {
  const {
    register,
    handleSubmit,
    getValues,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="deployment-form space-y-4" autoComplete="off" data-lpignore="true" data-form-type="other">
      ...
      <input type="password" name="fake-password" style={{ display: 'none' }} tabIndex={-1} autoComplete="new-password" aria-hidden="true" />
        ...
        <div className="form-field">
          ...
          />
          {errors['environment'] && (
            <p className="mt-1 text-xs text-red-400">{errors['environment']?.message}</p>
          )}
        </div>
      </div>
