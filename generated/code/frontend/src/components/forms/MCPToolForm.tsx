/**
 * MCPToolForm
 * Form component for creating/editing MCPTool
 */
...
const mcptoolSchema = z.object({
  ...
  isEntityModuleTool: z.boolean().optional(),
});

type MCPToolFormData = z.infer<typeof mcptoolSchema>;

interface MCPToolFormProps {
  mcptool?: MCPTool;
  onSubmit: (data: MCPToolFormData) => void;
  onValidate?: (data: MCPToolFormData) => void;
  onDelete?: () => void;
  onCancel?: () => void;
  isDeleting?: boolean;
...
/**
 * MCPToolForm Component
 */
export function MCPToolForm({ mcptool, onSubmit, onValidate, onDelete, onCancel, isDeleting, isValidating }: MCPToolFormProps) {
  const {
    register,
    handleSubmit,
    getValues,
  ...
  } = useForm<MCPToolFormData>({
    resolver: zodResolver(mcptoolSchema),
    defaultValues: mcptool || {},
  });

  // Reset form when mcptool changes
  useEffect(() => {
    if (mcptool) {
      reset(mcptool);
    } else {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="mcptool-form space-y-4" autoComplete="off" data-lpignore="true" data-form-type="other">
      ...
      <input type="password" name="fake-password" style={{ display: 'none' }} tabIndex={-1} autoComplete="new-password" aria-hidden="true" />
        ...
        <div className="form-field">
          <label htmlFor="isEntityModuleTool" className="flex items-center text-sm font-medium text-gray-300">
            <input
              id="isEntityModuleTool"
              type="checkbox"
              {...register('isEntityModuleTool')}
              className="mr-2"
              autoComplete="off"
              data-lpignore="true"
            />
          ...
          {errors['isEntityModuleTool'] && (
            <p className="mt-1 text-xs text-red-400">{errors['isEntityModuleTool']?.message}</p>
          )}
        </div>
      </div>
