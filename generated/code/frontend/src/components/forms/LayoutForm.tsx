/**
 * LayoutForm
 * Form component for creating/editing Layout
 */
...
interface LayoutFormProps {
  ...
  onValidate?: (data: LayoutFormData) => void;
  onDelete?: () => void;
  onCancel?: () => void;
  isDeleting?: boolean;
 ...
 */
export function LayoutForm({ layout, onSubmit, onValidate, onDelete, onCancel, isDeleting, isValidating }: LayoutFormProps) {
  const {
    register,
    handleSubmit,
    getValues,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="layout-form space-y-4" autoComplete="off" data-lpignore="true" data-form-type="other">
      ...
      <input type="password" name="fake-password" style={{ display: 'none' }} tabIndex={-1} autoComplete="new-password" aria-hidden="true" />
        ...
        <div className="form-field">
          <label htmlFor="layoutType" className="block text-sm font-medium text-gray-300 mb-1">
            layoutType
          </label>
          <input
            id="layoutType"
          ...
          />
          {errors['layoutType'] && (
            <p className="mt-1 text-xs text-red-400">{errors['layoutType']?.message}</p>
          )}
        </div>
      </div>
