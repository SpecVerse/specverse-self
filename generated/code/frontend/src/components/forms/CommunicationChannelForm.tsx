 ...
 * Form component for creating/editing CommunicationChannel
 */
...
interface CommunicationChannelFormProps {
  communicationchannel?: CommunicationChannel;
  onSubmit: (data: CommunicationChannelFormData) => void;
  onValidate?: (data: CommunicationChannelFormData) => void;
  onDelete?: () => void;
  onCancel?: () => void;
  isDeleting?: boolean;
...
/**
 * CommunicationChannelForm Component
 */
export function CommunicationChannelForm({ communicationchannel, onSubmit, onValidate, onDelete, onCancel, isDeleting, isValidating }: CommunicationChannelFormProps) {
  const {
    register,
    handleSubmit,
    getValues,
    ...
    defaultValues: communicationchannel || {},
  });

  // Reset form when communicationchannel changes
  useEffect(() => {
    if (communicationchannel) {
      reset(communicationchannel);
    } else {
      reset({});
    }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="communicationchannel-form space-y-4" autoComplete="off" data-lpignore="true" data-form-type="other">
      ...
      <input type="password" name="fake-password" style={{ display: 'none' }} tabIndex={-1} autoComplete="new-password" aria-hidden="true" />
        ...
        <div className="form-field">
          <label htmlFor="channelType" className="block text-sm font-medium text-gray-300 mb-1">
            channelType
          </label>
          <input
            id="channelType"
          ...
          />
          {errors['channelType'] && (
            <p className="mt-1 text-xs text-red-400">{errors['channelType']?.message}</p>
          )}
        </div>
      </div>
