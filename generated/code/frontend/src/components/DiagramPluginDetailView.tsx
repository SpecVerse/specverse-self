...
function DiagramPluginDetailView() {
  ...
  // Fetch all entities
  const { diagramplugins, isLoading: isLoadingDiagramPlugin } = useDiagramPlugin({ list: true });
  ...
  const modelData = useMemo(() => ({
    DiagramPlugin: (diagramplugins || []).map((item: any) => ({ id: item.id, data: item }))
  }), [diagramplugins]);
  
  // Entity selection
  const entities = diagramplugins || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.DiagramPlugin?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingDiagramPlugin) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'DiagramPlugin' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["DiagramPlugin"], name: 'DiagramPluginDetailView' },
    modelData,
    modelSchemas,
