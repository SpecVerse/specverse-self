...
function ExportDetailView() {
  ...
  const selectedEntity = useMemo(
    () => modelData.Export?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Export' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Export"], name: 'ExportDetailView' },
    modelData,
    modelSchemas,
    primaryModel: 'Export',
    selectedEntity,
