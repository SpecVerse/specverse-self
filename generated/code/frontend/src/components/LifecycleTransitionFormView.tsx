...
function LifecycleTransitionFormView() {
  ...
  const [lifecycleOptions, setLifecycleOptions] = useState<any[]>([]);
  ...
  useEffect(() => {
    fetch('/api/lifecycles').then(r => r.json()).then(d => setLifecycleOptions(d)).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/lifecycleTransitions/${id}` : '/api/lifecycleTransitions';
      ...
      navigate('/lifecycleTransitionlist');
    } catch (e: any) { setError(e.message); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!id || !confirm('Delete this lifecycleTransition?')) return;
    await fetch(`/api/lifecycleTransitions/${id}`, { method: 'DELETE' });
    navigate('/lifecycleTransitionlist');
  };
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} LifecycleTransition</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">fromState</label>
          <input
            type="text"
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">toState</label>
          <input
            type="text"
            value={form.toState ?? ''}
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/lifecycleTransitionlist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
