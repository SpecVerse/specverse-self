...
function AttributeFormView() {
  ...
  // Fetch single attribute if selected
  const { attribute, create, update, delete: deleteAttribute, validate, isDeleting, isValidating } = useAttribute(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all attributes for the list
  const { attributes, isLoading: listLoading } = useAttribute({ list: true });
  ...
  return (
    <div className="view-attributeformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Attribute' : 'Create Attribute'}
          </h2>
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No attributes yet</p>
              <p className="text-sm text-gray-500">Create your first attribute using the form above</p>
            </div>
          ) : (
                  ...
                  {attributes?.map((attribute: Attribute) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === attribute.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
