...
/**
 * EventFormView
 * Form view for creating and editing Event
 *
 * Model: Event
 * Type: form
 */
function EventFormView() {
  ...
  // Fetch single event if selected
  const { event, create, update, delete: deleteEvent, validate, isDeleting, isValidating } = useEvent(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all events for the list
  const { events, isLoading: listLoading } = useEvent({ list: true });
  ...
  return (
    <div className="view-eventformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              ...
              <p className="text-sm text-gray-500">Create your first event using the form above</p>
            </div>
          ) : (
                  ...
                  {events?.map((event: Event) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === event.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
