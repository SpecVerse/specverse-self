...
function CommunicationChannelDetailView() {
  ...
  // Entity selection
  const entities = communicationchannels || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.CommunicationChannel?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingCommunicationChannel) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'CommunicationChannel' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["CommunicationChannel"], name: 'CommunicationChannelDetailView' },
    modelData,
    modelSchemas,
