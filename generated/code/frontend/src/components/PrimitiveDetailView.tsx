...
function PrimitiveDetailView() {
  ...
  // Fetch all entities
  const { primitives, isLoading: isLoadingPrimitive } = usePrimitive({ list: true });
  ...
  const modelData = useMemo(() => ({
    Primitive: (primitives || []).map((item: any) => ({ id: item.id, data: item }))
  }), [primitives]);
  
  // Entity selection
  const entities = primitives || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.Primitive?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingPrimitive) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Primitive' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Primitive"], name: 'PrimitiveDetailView' },
    modelData,
    modelSchemas,
