...
function PrimitiveFormView() {
  ...
  // Fetch single primitive if selected
  const { primitive, create, update, delete: deletePrimitive, validate, isDeleting, isValidating } = usePrimitive(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all primitives for the list
  const { primitives, isLoading: listLoading } = usePrimitive({ list: true });
  ...
  const handleDelete = async () => {
    ...
    if (!confirm('Are you sure you want to delete this primitive?')) return;
  
    try {
      await deletePrimitive(selectedId);
      setSelectedId(null); // Clear selection after delete
    } catch (error) {
      console.error('Error deleting primitive:', error);
      alert('Failed to delete primitive');
    }
  };
  ...
  return (
    <div className="view-primitiveformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Primitive' : 'Create Primitive'}
          </h2>
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No primitives yet</p>
              <p className="text-sm text-gray-500">Create your first primitive using the form above</p>
            </div>
          ) : (
                  ...
                  {primitives?.map((primitive: Primitive) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === primitive.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
                      {/* Table cells will be dynamically generated based on model attributes */}
                      <td className="px-4 py-3 text-right">
                        <button
                          onClick={() => handleSelectPrimitive(primitive.id)}
