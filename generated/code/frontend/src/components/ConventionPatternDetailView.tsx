...
function ConventionPatternDetailView() {
  ...
  // Entity selection
  const entities = conventionpatterns || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.ConventionPattern?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingConventionPattern) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'ConventionPattern' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["ConventionPattern"], name: 'ConventionPatternDetailView' },
    modelData,
    modelSchemas,
