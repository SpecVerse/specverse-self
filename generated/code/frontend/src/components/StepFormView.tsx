...
/**
 * StepFormView
 * Form view for creating and editing Step
 *
 * Model: Step
 * Type: form
 */
function StepFormView() {
  ...
  // Fetch single step if selected
  const { step, create, update, delete: deleteStep, validate, isDeleting, isValidating } = useStep(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all steps for the list
  const { steps, isLoading: listLoading } = useStep({ list: true });
  ...
  return (
    <div className="view-stepformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Step' : 'Create Step'}
          </h2>
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No steps yet</p>
              <p className="text-sm text-gray-500">Create your first step using the form above</p>
            </div>
          ) : (
                  ...
                  {steps?.map((step: Step) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === step.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
