...
function StepFormView() {
  ...
  useEffect(() => {
    fetch('/api/operations').then(r => r.json()).then(d => setOperationOptions(d)).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/steps/${id}` : '/api/steps';
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} Step</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        ...
        <div>
          ...
          <input
            type="number"
            value={form.ordinal ?? ''}
            onChange={e => setForm({...form, ordinal: parseInt(e.target.value)})}
            className="w-full px-3 py-2 border rounded-md focus:ring-blue-500 focus:border-blue-500"
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/steplist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
