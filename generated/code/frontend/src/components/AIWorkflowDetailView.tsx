...
function AIWorkflowDetailView() {
  ...
  const selectedEntity = useMemo(
    () => modelData.AIWorkflow?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingAIWorkflow) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'AIWorkflow' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["AIWorkflow"], name: 'AIWorkflowDetailView' },
    modelData,
    modelSchemas,
