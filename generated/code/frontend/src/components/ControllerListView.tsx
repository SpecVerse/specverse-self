  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link to="/controllerform" className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">
          + New Controller
        </Link>
      </div>
      ...
      {items.length === 0 ? (
        <div className="text-center py-12 text-gray-500">
          <p className="text-lg">No controllers yet</p>
          <p className="text-sm mt-1">Create your first controller to get started</p>
        </div>
      ) : (
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">parentModel</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase whitespace-nowrap">hasAssociationOps</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{formatCell(item.parentModel)}</td>
                  <td className="px-4 py-3 text-sm text-gray-900 whitespace-nowrap">{formatCell(item.hasAssociationOps)}</td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <Link to={`/controllerdetail?id=${item.id}`} className="text-blue-600 hover:text-blue-800 mr-3">View</Link>
                    <Link to={`/controllerform?id=${item.id}`} className="text-green-600 hover:text-green-800">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>
