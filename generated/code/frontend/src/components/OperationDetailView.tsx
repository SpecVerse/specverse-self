...
function OperationDetailView() {
  ...
  const { steps, isLoading: isLoadingStep } = useStep({ list: true });
  ...
  const modelData = useMemo(() => ({
    ...
    Step: (steps || []).map((item: any) => ({ id: item.id, data: item }))
  }), [operations, attributes, events, steps]);
  
  // Entity selection
  const entities = operations || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.Operation?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingOperation || isLoadingAttribute || isLoadingEvent || isLoadingStep) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'Operation' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["Operation","Attribute","Event","Step"], name: 'OperationDetailView' },
    modelData,
    modelSchemas,
