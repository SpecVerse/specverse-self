...
function SchemaFragmentDetailView() {
  ...
  // Entity selection
  const entities = schemafragments || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.SchemaFragment?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingSchemaFragment) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'SchemaFragment' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["SchemaFragment"], name: 'SchemaFragmentDetailView' },
    modelData,
    modelSchemas,
