...
/**
 * DeploymentFormView
 * Form view for creating and editing Deployment
 *
 * Model: Deployment
 * Type: form
 */
function DeploymentFormView() {
  const [selectedId, setSelectedId] = useState<string | null>(null);
  
  // Fetch single deployment if selected
  const { deployment, create, update, delete: deleteDeployment, validate, isDeleting, isValidating } = useDeployment(
    selectedId ? { id: selectedId } : {}
  );
  
  // Fetch all deployments for the list
  const { deployments, isLoading: listLoading } = useDeployment({ list: true });
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDeployment(selectedId);
  ...
  return (
    <div className="view-deploymentformview min-h-screen bg-slate-900 text-gray-200">
      {/* Content */}
      <div className="max-w-7xl mx-auto p-6 space-y-6">
        ...
        <div className="curved-form-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          <h2 className="text-xl font-semibold text-gray-200 mb-1">
            {selectedId ? 'Edit Deployment' : 'Create Deployment'}
          </h2>
        ...
        <div className="curved-list-section bg-slate-800 rounded-lg border border-slate-700 p-6">
          {listLoading ? (
            ...
            <div className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center">
              <p className="text-gray-400 mb-1">No deployments yet</p>
              <p className="text-sm text-gray-500">Create your first deployment using the form above</p>
            </div>
          ) : (
                  ...
                  {deployments?.map((deployment: Deployment) => (
                    <tr
                      ...
                      className={`border-b border-slate-700 transition-colors ${
                        selectedId === deployment.id ? 'bg-slate-700/50' : 'hover:bg-slate-700/30'
                      }`}
                    >
...
export default DeploymentFormView;
