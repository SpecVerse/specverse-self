...
function CodeTemplateFormView() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = id ? `/api/codeTemplates/${id}` : '/api/codeTemplates';
      ...
      navigate('/codeTemplatelist');
    } catch (e: any) { setError(e.message); }
    finally { setSaving(false); }
  };

  const handleDelete = async () => {
    if (!id || !confirm('Delete this codeTemplate?')) return;
    await fetch(`/api/codeTemplates/${id}`, { method: 'DELETE' });
    navigate('/codeTemplatelist');
  };
  ...
  return (
    <div className="p-6 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">{id ? 'Edit' : 'New'} CodeTemplate</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow rounded-lg p-6 space-y-4">
        <div>
          ...
          <select
            value={form.entityModuleId ?? ''}
            onChange={e => setForm({...form, entityModuleId: e.target.value})}
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">basePath</label>
          <input
            type="text"
            value={form.basePath ?? ''}
        ...
        <div className="flex justify-between pt-4">
          ...
          <div className="space-x-2">
            <button type="button" onClick={() => navigate('/codeTemplatelist')} className="px-4 py-2 bg-gray-200 text-gray-700 rounded hover:bg-gray-300">Cancel</button>
