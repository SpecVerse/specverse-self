...
function CapabilityMappingDetailView() {
  ...
  // Entity selection
  const entities = capabilitymappings || [];
  useEffect(() => {
  ...
  const selectedEntity = useMemo(
    () => modelData.CapabilityMapping?.find((e: any) => e.id === selectedEntityId) || null,
  ...
  if (isLoadingCapabilityMapping) {
    return <div className="p-4">Loading...</div>;
  }
  
  // Detect pattern
  const pattern = patternAdapter.detectPattern({ type: 'detail', model: 'CapabilityMapping' });
  ...
  const context = {
    pattern,
    viewSpec: { type: 'detail', model: ["CapabilityMapping"], name: 'CapabilityMappingDetailView' },
    modelData,
    modelSchemas,
