...
The following domains are not yet covered by .specly schema primitives. These are roadmap items - planned extensions to the schema and inference engine - not fundamental architectural limitations. The .specly format is designed to be extended incrementally.
...
What's needed - new schema primitives for measures, dimensions, KPIs, and data aggregation:
...
SpecVerse describes internal services well but has no way to specify how your system talks to the outside world - external API contracts, webhook handling, retry policies, circuit breakers, authentication with third-party services. This requires new schema primitives for external service definitions and connection policies.
