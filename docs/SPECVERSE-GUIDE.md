...
For complex business logic, you can write **Quint action specifications** alongside your `.specly` file. Quint actions are formally verifiable and transpile to TypeScript:
...
1. **Quint actions** — if a matching action exists, transpile it (formally verified)
2. **Convention patterns** — 15 common patterns generate real code
3. **Stub methods** — compile but throw `Not implemented` at runtime
